            ...
            for scenario in ['cross']:
                ...
                for ft in [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]:
                    ...
                    for n in [1, 2, 3]:
                        ...
                        if ft != 0:
                            # keep = [i for i in range(len(posCV.vocabulary_)) if np.sum(all_feats, axis=0)[i] >= ft]
                            keep = [posCV.get_feature_names()[i] for i in range(len(posCV.vocabulary_)) if
                             np.sum(all_feats, axis=0)[i] >=ft]
                        ...
                        for i in range(FLAGS.run_times):
                            clf_stylo = None
                            clf_stylo = SGDClassifier(loss=loss, max_iter=2000, tol=1e-3, class_weight='balanced',
                                                      n_jobs=-1)
                        ...
                        print(FLAGS.guardian_case, '\t', FLAGS.scenario, "\t", ft, '\t', n, '\t', num_feat, '\t\t', end=' ')
