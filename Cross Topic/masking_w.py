...
def main(argv):
    suffix = "same"
    # FLAGS.dataset_name, FLAGS.embed_path, FLAGS.update_emb, FLAGS.epochs, FLAGS.lower_case
    print("case \t Scenario \t K \t Ft \t n \t num_feats \t train \t\t\t valid \t\t\t test")
    # with open('/' + str(os.path.basename(__file__).split('.')[0]) + '.txt', '+w') as resFile:
    with open('D:/Malik/myResults/Hyper/' + str(os.path.basename(__file__).split('.')[0]) + '_' + suffix + '.txt', '+w') as resFile:
