...
class CharFeats():
    ...
    def update(self, doc):
        ...
        self.alphaCounts       = dict([(a  , len(re.findall(re.compile('['  +a+A+']'), doc))) for a,A in zip(string.ascii_lowercase,string.ascii_uppercase)] )  # 26 feats ie. case insensitive
        self.specialCounts     = dict([(sym, len(re.findall(re.compile('[\\'+sym+']'), doc))) for sym in "#<>%|{}[]/\@~+-*=$&^()_`"] ) # 24 feats
...
class WordFeats(object):
    ...
    def update(self, doc):
        ...
        self.vocabRichness = self.Yolks_k(doc) # Yule's K measure : https://gist.github.com/magnusnissel/d9521cb78b9ae0b2c7d6
...
class SyntacticFeats(object):
    def __init__(self, name='Syntactic'):
        ...
        self.fnWords = ['a', 'about', 'above', 'across', 'after', 'afterwards', 'again', 'against',
                   ...
                   'will', 'with', 'within', 'without', 'would', 'yes', 'yet', 'you', 'your', 'yours', 'yourself', 'yourselves']
...
def main(argv):
    ...
    with open('/' + str(os.path.basename(__file__).split('.')[0]) + '.txt', '+w') as resFile:
        ...
        resFile.write("case \t Scenario \t num_feats \t train \t valid \t test \n")

        # print("Case \t Scenario \t train \t\t valid \t\t test")
        for case in range(1,13):
            ...
            for scenario in ['same']:
                ...
                train_x_stylo = extract_all_feats(train_x)
                # posCV.fit([p_doc.posFeats.tags for p_doc in train_x_stylo])

                train_x_stylo = get_all_feats(train_x_stylo)
                # train_x_stylo = all_feats(train_x)

                valid_x_stylo = extract_all_feats(valid_x)
                valid_x_stylo = get_all_feats(valid_x_stylo)
                # valid_x_stylo = all_feats(valid_x)

                tests_x_stylo = extract_all_feats(tests_x)
                tests_x_stylo = get_all_feats(tests_x_stylo)
                ...
                num_feat = np.shape(train_x_stylo)[1]
                ...
                for i in range(FLAGS.run_times):
                    clf_stylo = None
                    clf_stylo = SGDClassifier(loss=loss, max_iter=2000, tol=1e-3, class_weight='balanced', n_jobs=-1)
                    clf_stylo.fit(train_x_stylo, y_train)

                    train_score = balanced_accuracy_score(y_train,clf_stylo.predict(train_x_stylo))
                    # print("Training acc:\t {:.2f}".format(train_score))
                    train.append(train_score)

                    valid_score = balanced_accuracy_score(y_valid, clf_stylo.predict(valid_x_stylo))
                    # print("Validation acc:\t {:.2f}".format(valid_score))
                    valid.append(valid_score)

                    tests_score = balanced_accuracy_score(y_tests, clf_stylo.predict(tests_x_stylo))
                    # print("Testing acc:\t {:.2f}".format(tests_score))
                    tests.append(tests_score)

                resFile.write(
                    f'{FLAGS.guardian_case} \t {FLAGS.scenario}  \t {num_feat} \t'
                )
                ...
                print(FLAGS.guardian_case, '\t\t', FLAGS.scenario, "\t", num_feat, '\t', end=' ')
